...
public class Connected extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        statusComet = (TextView) findViewById(R.id.textView);
    ...
    public void Cometwork (View view)
    {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Confirm");
        builder.setMessage("Are you sure?");
        ...
        builder.setPositiveButton("YES", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                // Do nothing but close the dialog
                try {
                    String outMsg = "goto goal3"+ System.getProperty("line.separator");
                    txt = txt+outMsg;
                    textResponse.setText(txt);
                    statusComet.setText("Sending to Comet");
    ...
    public void Linework (View view)
    {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Confirm");
        builder.setMessage("Are you sure?");
        ...
        builder.setPositiveButton("YES", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                try {
                    ...
                    txt = txt+outMsg;
                    textResponse.setText(txt);
                    statusServiceLine.setText("Sending to Service Line");
                    SystemClock.sleep(100);
                    out.write(outMsg);
                    out.flush();
                    Log.i("TcpClient", "sent: " + outMsg);
    ...
    public void Panddwork (View view)
    {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Confirm");
        builder.setMessage("Are you sure?");
        ...
        builder.setPositiveButton("YES", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                try {
                    ...
                    txt = txt+outMsg;
                    textResponse.setText(txt);
                    statusPandd.setText("Sending to AS/RS");
                    SystemClock.sleep(100);
                    out.write(outMsg);
                    out.flush();
                    Log.i("TcpClient", "sent: " + outMsg);
                ...
                while (socket.isConnected()) {
                    ...
                    else if(stone.equals(inMsg))
                    {
                        Connected.this.runOnUiThread(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                statusServiceLine.setText("Arrived station Service Line");
                            }
                        });
                    }
                    else if(sttwo.equals(inMsg))
                    {
                        Connected.this.runOnUiThread(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                statusPandd.setText("Arrived station AS/RS");
                            }
                        });
                    }
                    else if(stthree.equals(inMsg))
                    {
                        Connected.this.runOnUiThread(new Runnable() {

                            @Override
                            public void run() {
                                statusComet.setText("Arrived station Comet");
                            }
                        });
                    }
                    else if((statone.equals(inMsg))|| (staone.equals(inMsg)))
                    {
                        Connected.this.runOnUiThread(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                statusServiceLine.setText("Receive from Service Line");
                            }
                        });
                    }
                    else if(stattwo.equals(inMsg)||(statwo.equals(inMsg)))
                    {
                        Connected.this.runOnUiThread(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                statusPandd.setText("Receive from AS/RS");
                            }
                        });
                    }
                    else if(statthree.equals(inMsg)||(stathree.equals(inMsg)))
                    {
                        Connected.this.runOnUiThread(new Runnable() {

                            @Override
                            public void run() {
                                statusComet.setText("Receive from Comet");
                            }
                        });
                    }
                    else if((beforecommand.equals(statone)) && (error.equals(inMsg)))
                    {
                        Connected.this.runOnUiThread(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                statusServiceLine.setText("Error: Failed going to Service Line");
                            }
                        });
                    }
                    else if((beforecommand.equals(stattwo)) && (error.equals(inMsg)))
                    {
                        Connected.this.runOnUiThread(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                statusPandd.setText("Error: Failed going to AS/RS");
                            }
                        });
                    }
                    else if((beforecommand.equals(statthree)) && (error.equals(inMsg)))
                    {
                        Connected.this.runOnUiThread(new Runnable() {

                            @Override
                            public void run() {
                                statusComet.setText("Error: Failed going to Comet");
                            }
                        });
                    }
                    /*else
